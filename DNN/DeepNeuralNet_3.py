...
def loadData():
data = np.loadtxt("poker-hand-testing.data",delimiter=',')

nbExample = np.shape(data)[0]
nbTrainingExample = int(0.7*nbExample)

nbTestExample = nbExample - nbTrainingExample

print("dataset "+str(nbExample)+" con "+str(nbTrainingExample)
                    +" training y "+str(nbTestExample)+" test example")

X_train = data[0:nbTrainingExample,:-1]  # 70 % pas 70
Y_train = data[0:nbTrainingExample,-1]

X_test = data[nbTrainingExample:,:-1]
Y_test = data[nbTrainingExample:,-1]

Y_test_encoded = np.zeros((nbTestExample,10))
Y_train_encoded = np.zeros((nbTrainingExample,10))

#one hot enconding Y
for i in range(0,nbTrainingExample):
    Y_train_encoded[i,int(Y_train[i])] = 1
for i in range(0,nbTestExample):
    Y_test_encoded[i,int(Y_test[i])] = 1
print("finished "+str(np.shape(X_train)))
